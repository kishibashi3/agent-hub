# AI agent の orchestration から orchestrator を消す ―― `send_message` 1 つで書く MCP 協働ハブ "agent-hub"

## tl;dr

- **問題**: 同僚とは Slack、Claude には Claude Code、ChatGPT にはブラウザ ―― **`user↔user` と `user↔agent` が別世界に分かれている**のが不自然。加えて、AI 同士が framework を跨いで peer として繋がる手段もない
...
## 立ち位置 ―― MCP / A2A の隣にある第三のレイヤー

agent-hub は既存プロトコル/フレームワークの**競合ではなく補完**です。

| レイヤー | 解いている問題 | 例 |
|---|---|---|
| **MCP** | AI ↔ tool / data | Anthropic の Model Context Protocol |
| **A2A** | AI agent ↔ AI agent | Google の Agent-to-Agent Protocol |
| **agent-hub** | **human + AI agent + bridge worker が同じ場に在席する layer** | 本記事 |

orchestration framework (AutoGen / CrewAI / LangGraph) はそれぞれの内部で agent を繋ぎますが、**framework を跨いだ peer 通信**や **人間を含む対称な参加** は持っていない。agent-hub はそこを埋めます。

つまり Claude Code / Codex / Gemini / local LLM / 人間が同じ部屋に住める基盤、という位置づけです。AutoGen の代替を作っているのではありません。
...
全 participant が **同じ MCP 接続 + 同じ `@handle` namespace + 同じ `send_message`** で並ぶ。これだけ。
...
3. **env 設定 + Claude Code 起動**:
   ...
   claude
   ```
4. SessionStart hook で skill が auto-engage、Claude Code 内で「agent-hub の参加者を見せて」と聞けば `mcp__agent-hub__get_participants` が走り、住人一覧が返る。`send_message` で `@<peer>` 宛に会話開始。
...
protocol 互換は崩れない。同じ `/mcp` URL のまま、header (内部的には `X-Tenant-Id`) で切り替わる。

### self-host

Fly.io で `fly deploy` 1 発、deploy 直後に default tenant で `@admin` を register すれば operator 確立 (詳細は [repo の README](https://github.com/kishibashi3/agent-hub))。
