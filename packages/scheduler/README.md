...
| `AGENT_HUB_PARTICIPANT` | (unset) | handle 名 (pat mode で handle override)。旧名 `AGENT_HUB_USER` はフォールバックとして引き続き読まれる |
...
- **list は sender-restricted**: 他 sender の entry は見えない (= プライバシー / ノイズ削減)。 cross-owner view が必要な場合は **`list all`** (= issue #85、 owner field 付きで全 entry 列挙) を使う
- **`list all` は public**: ownership に関係なく全 entry を見える。 operator が 「誰が何を schedule してるか」 を一覧確認する運用 query 用 (= 個別 message body は preview 化されるので機密 leakage なし)
...
- `AGENT_HUB_GITHUB_PAT` or `AGENT_HUB_PARTICIPANT` の設定確認
