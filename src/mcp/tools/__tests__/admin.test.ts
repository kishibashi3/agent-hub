...
describe('admin ツール', () => {
  ...
  beforeEach(async () => {
    ...
    await handleRegister(db, { name: 'alice' }, 'alice', 'alice-gh');
    await handleRegister(db, { name: 'bob' }, 'bob', 'bob-gh');
  });

  describe('権限チェック (共通)', () => {
    it('@admin 以外は delete_user を呼べない', async () => {
      const r = await handleDeleteUser(db, { name: 'alice' }, 'bob');
    ...
    it('@admin 以外は get_user_history を呼べない', async () => {
      const r = await handleGetUserHistory(db, { name: 'alice' }, 'bob');
      expect(r.isError).toBe(true);
      const body = JSON.parse(r.content[0].text as string);
      expect(body.error).toBe('forbidden');
    });
  ...
  describe('delete_user', () => {
    it('admin として peer を soft delete できる', async () => {
      const r = await handleDeleteUser(db, { name: 'alice' }, '@admin');
      ...
      const row = db
        .prepare(
          'SELECT name, deleted_at FROM participants WHERE name = ?'
        )
        .get('@alice') as { name: string; deleted_at: string | null } | undefined;
      expect(row).toBeDefined();
      expect(row?.deleted_at).not.toBeNull();
    });

    it('@admin 自身は削除できない', async () => {
      const r = await handleDeleteUser(db, { name: 'admin' }, '@admin');
      expect(r.isError).toBe(true);
      const body = JSON.parse(r.content[0].text as string);
    ...
    it('存在しない peer は削除できない', async () => {
      const r = await handleDeleteUser(db, { name: 'ghost' }, '@admin');
      expect(r.isError).toBe(true);
      const body = JSON.parse(r.content[0].text as string);
    ...
    it('soft delete 後も送信済メッセージは残る (FK 整合)', async () => {
      ...
      const afterMsgs = db
        .prepare('SELECT COUNT(*) as c FROM messages WHERE sender = ?')
        .get('@alice') as { c: number };
      expect(afterMsgs.c).toBe(1);
    });

    it('name バリデーション: 空は拒否', async () => {
      const r = await handleDeleteUser(db, { name: '' }, '@admin');
      expect(r.isError).toBe(true);
    });

    it('@ 付き / なし両方受け付ける', async () => {
      const r = await handleDeleteUser(db, { name: '@bob' }, '@admin');
      expect(r.isError).toBeUndefined();
      const body = JSON.parse(r.content[0].text as string);
  ...
  describe('get_user_history', () => {
    beforeEach(async () => {
      ...
      await handleSendMessage(db, { to: 'bob', message: 'hi bob' }, 'alice');
      await handleSendMessage(db, { to: 'alice', message: 'hi alice' }, 'bob');
      await handleSendMessage(db, { to: 'admin', message: 'cc admin' }, 'alice');
    });

    it('admin として peer の履歴 (sent + received) を取得できる', async () => {
      const r = await handleGetUserHistory(db, { name: 'alice' }, '@admin');
      expect(r.isError).toBeUndefined();
      const body = JSON.parse(r.content[0].text as string);
    ...
    it('limit を効かせられる', async () => {
      const r = await handleGetUserHistory(db, { name: 'alice', limit: 1 }, '@admin');
      expect(r.isError).toBeUndefined();
      const body = JSON.parse(r.content[0].text as string);
    ...
    it('存在しない peer は拒否', async () => {
      const r = await handleGetUserHistory(db, { name: 'ghost' }, '@admin');
      expect(r.isError).toBe(true);
    });

    it('limit > 500 は zod で拒否', async () => {
      const r = await handleGetUserHistory(db, { name: 'alice', limit: 9999 }, '@admin');
      expect(r.isError).toBe(true);
    });
  });
});
