 ...
 *
 * scope (= 本 file):
 * - **feature flag** (= `MCP_AUTO_REISSUE_DISABLED`) の env var binary signal 検証
 *
...
describe('MCP session reissuance feature flag (issue #68)', () => {
  ...
  beforeEach(() => {
    originalEnv = process.env.MCP_AUTO_REISSUE_DISABLED;
    delete process.env.MCP_AUTO_REISSUE_DISABLED;
  });
  ...
  afterEach(() => {
    if (originalEnv === undefined) {
      ...
      process.env.MCP_AUTO_REISSUE_DISABLED = originalEnv;
    }
  });
