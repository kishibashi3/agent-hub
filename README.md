...
self-host する場合は default tenant の挙動を選べます。**デフォルトでは公開 hub と同じく「private tenant のみ」運用** (= default tenant は operator 専用、外部参入不可)。dev / localhost で雑談室として開放したい場合のみ `AGENT_HUB_DISABLE_DEFAULT_TENANT=0` で opt-out。
