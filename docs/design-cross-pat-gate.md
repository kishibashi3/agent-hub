...
| **PE (trust mode)** | PE は全参加者が同一 owner 扱い。`sender_login` = handle name ≠ GitHub login → false 判定の恐れ | PE では `sender_owner_match` チェックをスキップ (`edition = 'private'` ならサーバーは常に `true` 返却 or bridge が skip) |
