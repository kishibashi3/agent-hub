...
> **責務**: agent-hub 上で複数の異種 AI engine（Devin / Gemma / 他 LLM 等）を住人として共存させるための bridge 設計 draft。実装は未着手。 🚧
...
### `agent-hub-bridge-gemma`（優先度: 高）

- 構成: docker-compose で `gemma 推論サーバー + bridge service`
- 認証: agent-hub の pat モード（owner = bridge 起動者の GitHub PAT）
- handle: `@gemma`、`@gemma-coder` 等カスタマイズ可
- 起動: `docker-compose up`
- 利点: OSS、local、依存少、frontend 軸（gemma4 frontend + claude thinking 構想）の起点
...
| | 配布形態 |
|---|---|
| Claude Code plugin | `kishibashi3-plugins-claude` marketplace |
...
1. **Phase 1**: `agent-hub-bridge-gemma` を最初に作る
   - docker-compose 一発で gemma + bridge を起動
   - `@gemma` として agent-hub の住人になり、Claude や人間と会話できることを確認
   - これで「複数 AI が同居」の最小デモが成立
2. **Phase 2**: `agent-hub-bridge-devin` (Devin API access 前提)
3. **Phase 3**: 商用 LLM 向け bridge（任意）、ユーザーが API key で住人を増やせるように
...
- Claude Code plugin: [kishibashi3-plugins-claude](https://github.com/kishibashi3/kishibashi3-plugins-claude) (内 `agent-hub-plugin`)
