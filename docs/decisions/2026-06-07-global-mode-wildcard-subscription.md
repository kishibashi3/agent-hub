...
function uriMatchesSubscription(
  subscribedUri: string,
  notifyUri: string,
  tenantDomain: string  // NOTE: 現在未使用。tenant スコープは呼び出し元の session コンテキストで既に適用済み。
                        // Phase 1 (URI に tenant 含める) 実装時に cross-tenant ガードとして活用予定。
): boolean {
  // exact match
...
- **自己宣言リスク**: 現在の `register` API は `mode` フィールドを participant が自己申告できる。悪意ある participant が `mode: "global"` を自己設定して wildcard subscribe を取得できてしまう。**軽減策**: `resources/subscribe` での mode チェックに加え、`register` / `update_participant` 時に `mode: "global"` の設定を operator 権限（特定の API key または admin フラグ）に制限する enforcement を実装 issue として別途起票する（実装は本 ADR のスコープ外）。短期的には operator が `mode: "global"` を付与するプロセスを運用ルールで管理する。
