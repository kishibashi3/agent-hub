...
- **Status 欄の粒度**: `**Status**` は上記テンプレートと同じ**単一ラベル** (`Proposed` / `Accepted` / `Adopted (consensus)` / `Deprecated` / `Superseded by ADR-NNN`) のみを書く。補足の叙述が必要な場合は frontmatter 直下に `> **Status note**: ...` の callout として置く。理由: `## 既存 ADR` 一覧表は単一ラベル前提で、本体に叙述が入ると粒度が食い違う。
