...
- 現在の最新: **ADR-007** (`2026-06-06-bridge-memory-interface.md`)
...
- **private repo path の参照**: 公開側から辿れない path (`agent-hub-roles-kaz/...` 等) を参照するときは `(private archive)` を付記し、**公開 issue / PR を併記**して OSS 側の読者が文脈を追えるようにする。例: `` `agent-hub-roles-kaz/chaos-research/...md` (private archive; public context: #329 / #364) ``。対応する公開 issue / PR が無い場合は、公開側に実体があれば公開 URL を併記する (例: `agent-hub-roles-kaz/admin/CLAUDE.md` (private fork; public template: [agent-hub-roles/admin/CLAUDE.md](https://github.com/kishibashi3/agent-hub-roles/blob/main/admin/CLAUDE.md)))。公開側に実体が無い場合は `(private; 公開 repo 外)` と付記し、**壊れた相対リンクにはしない** (素の code span + 注記に落とす)。
...
|---|---|---|---|---|
| ADR-001 | [2026-05-18-peer-mesh-architecture-decision.md](./2026-05-18-peer-mesh-architecture-decision.md) | Peer-Mesh Architecture with Transparent Asymmetry | Adopted (consensus) | ecosystem |
...
| ADR-007 | [2026-06-06-bridge-memory-interface.md](./2026-06-06-bridge-memory-interface.md) | Bridge Memory Interface — context persistence across sessions | Proposed | ecosystem |
