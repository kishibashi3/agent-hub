...
| **Full archive** | `BRIDGE_FULL_ARCHIVE`（任意の非空値）設定時 | 会話全文（JSONL） | 重要処理時のみ |
...
<workdir>/
  ...
  archive/
    YYYY-MM-DDTHH-MM-UTC.jsonl  ← フルアーカイブ（オプション、ファイル名は UTC）
```
...
2. `BRIDGE_FULL_ARCHIVE`（任意の非空値）環境変数が設定されている場合、会話全文を `archive/` に保存する
...
- bridge-claude の `src/agent_hub_bridges/claude/worker.py` で compaction hook を実装する（issue #131）
