...
| ⚠️ | `presence` / `register` を `inbox://` scheme の query 値に同居させると、「メッセージ受信箱」と「presence 通知チャネル」が同一 scheme に混在して意味論が曖昧になる (α案では `presence://` を独立 scheme にすることで明確に分離) |
...
> **Phase 2 の atomic 移行要件**: Phase 2 で `notifyResourceUpdated` の呼び出し側を `inbox://kaz/alice/message` 等のサブパス URI に切り替える際は、以下 3 点を **同一 PR 内で atomic に** 変更しなければならない。部分適用すると旧クライアントへの push が届かなくなる。
> 1. **URI 形式変更**: `notifyResourceUpdated("inbox://kaz/alice")` → `notifyResourceUpdated("inbox://kaz/alice/message")` 等、push 側の全呼び出し箇所
> 2. **prefix match 導入**: `selectNotificationTargets` の `subscribedUris.has(uri)` を `uriMatches(subscribedUri, notifyUri)` に置き換え (§3.1(e) 参照)
> 3. **リソース一覧更新**: `listResources` が返す URI も新形式に更新し、新規クライアントが正しい URI を取得できるようにする
