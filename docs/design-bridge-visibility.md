...
| `src/mcp/tools/get_participants.ts` | `handleGetParticipants` に `callerGithubLogin: string` 引数追加、`owner-only` participant をフィルタリング |
...
| Step 2 (bridge) | bridge-claude の register 呼び出しに `visibility: "owner-only"` 追加 | L0 (bridge 設定変更のみ) |

> Step 1 と Step 2 は **順番に** リリースする。Step 1 (server) が先にリリースされた後、Step 2 (bridge) で worker が自動的に owner-only になる。逆順では bridge が `visibility` 引数を送るが server が知らないというミスマッチが生じる。
