...
> **Step 1 と Step 2 の順序**: Step 1 (server) が先にリリースされた後、Step 2 (bridge) で worker が自動的に owner-only になる。逆順では bridge が `visibility` 引数を送るが server が知らないというミスマッチが生じる。Step 1 (server) が先にリリースされた後、Step 2 (bridge) で worker が自動的に owner-only になる。逆順では bridge が `visibility` 引数を送るが server が知らないというミスマッチが生じる。
