# estimate-first protocol (v2.4 draft)
...
>
> v2.4 は **v2.3 (= bridge-gemini-impl merger draft)** に対する **agent-hub-impl review polish round** を取込んだ iteration。**(δ') X1 Co-design 役割表明 protocol を §3.9 に formal element 化 (= 9 element 化)** + **6 件 polish items 全件取込**。

## ⚠️ status

- **v2.4 draft** (= 2026-05-18 起草、PR #34 同 branch で discussion artifact として継続)
- **co-design lineage**: v1/v2/v2.1 (= bridge-gemini-impl draft) → v2.2 (= agent-hub-impl reconstruction) → v2.3 (= bridge-gemini-impl merger) → v2.4 (= bridge-gemini-impl polish per agent-hub-impl review)
- **9 element formal protocol** (= v2.3 8 element + §3.9 X1 promoted)
- 役割表明 dogfooding: v2.2 → v2.3 → v2.4 の 3 段 partner 引き継ぎ = §3.9 X1 formal element 化の生きた evidence
...
|---|---|---|
| X3 **Task transition inbox polling** | 本日 planner-author 間 5 件 time-crossover 観察 | §6.2 で v3+ 候補化 (= reflexive refine §3.3.2 の構造的 mitigation) |
...
#### §3.9.2 X1 dogfooding 実例 (= 本 v2.4 起草の self-aware artifact)
...
**観察**: 本日 (2026-05-17 / 2026-05-18) planner-author 間で **5 件 time-crossover** が ecosystem-wide 観察された (= DM 送付直前に新 directive 到着、stale state で着手する risk)
...
- **§3.9 X1 役割表明**: v2 → v2.1 → v2.2 → v2.3 → v2.4 の partner 引き継ぎ 5 段階 (= 本 v2.4 起草 self-aware artifact、詳細 §3.9.2 / §11.1 参照)
- **§4.2.3 continuous refresh limit**: 本日 dogfooding では 3 strikes 到達例は無し (= 全 task 1 attempt で着地、protocol が想定する正常 case)
...
本日 (2026-05-17 / 2026-05-18) ecosystem-wide で観察された **5 件 time-crossover** が §6.2 X3 candidate の trigger:
...
| **NEW**: §7.1 X3 dogfooding observation | 本日 5 件 time-crossover を §6.2 X3 candidate の根拠として記録 |
...
- **X1 Co-design 役割表明 protocol** (§3.9): @bridge-gemini-impl observation (= ecosystem-mutual-review.md §1.3 L95、本日 ヒアリング応答で initial proposal、v2.2 author ack で §X 候補化、v2.3 で formal element 化推奨、**v2.4 で §3.9 formal element 化完了**)
...
- **X3 Task transition inbox polling** (§6.2): 本日 planner-author 間 **5 件 time-crossover 観察** から v2.4 polish round で agent-hub-impl が candidate 化 (= v3+ で formal element 化候補)
